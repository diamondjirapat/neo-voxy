...
public class AsyncNodeManager {
    ...
    static {
        try {
            ...
            RESULT_CACHE_1_HANDLE = MethodHandles.lookup().findVarHandle(AsyncNodeManager.class, "resultCache1",
                    SyncResults.class);
            RESULT_CACHE_2_HANDLE = MethodHandles.lookup().findVarHandle(AsyncNodeManager.class, "resultCache2",
                    SyncResults.class);
    ...
    private final IntOpenHashSet tlnIdChange = new IntOpenHashSet();// "Encoded" add/remove id, first bit indicates if
                                                                    // its add or remove, 1 is add
    // Top bit indicates clear or reset
    private final IntOpenHashSet cleanerIdResetClear = new IntOpenHashSet();// Tells the cleaner if it needs to clear
                                                                            // the id to 0, or reset the id to the
                                                                            // current frame
    ...
    public AsyncNodeManager(int maxNodeCount, IGeometryData geometryData, RenderGenerationService renderService) {
        ...
        // Note: geometry data is the data store/source, not the management, it is just
        // a raw store of data
        // it MUST ONLY be accessed on the render thread
        // AsyncNodeManager will use an AsyncGeometryManager as the manager for the data
        // store, and sync the results on the render thread
    ...
    private void run() {
        if (this.workCounter.get() <= 0) {
            // TODO: here, instead of parking, we can do more work on other sub-tasks such
            // as filtering the mesh build queue
            LockSupport.park();
            if (this.workCounter.get() <= 0 || !this.running) {// No work
                return;
            }
            // This is a funny thing, wait a bit, this allows for better batching, but this
            // thread is independent of everything else so waiting a bit should be mostly ok
            try {
                Thread.sleep(10);
        ...
        // Run in a loop, process all the input events, collect the output events merge
        // with previous and publish
        // note: inner event processing is a loop, is.. should be synced to
        // attomic/volatile variable that is being watched
        // when frametime comes around, want to exit out as quick as possible, or make
        // the event publishing
        // "effectivly immediately", that is, atomicly swap out the render side event
        // updates
        ...
        // if (old == null) {cleanAllEventsUpToThisPoint();}//(i.e. clear any buffers or
        // maps containing data revolving around uncommited render thread data events)

        // this creates a lock free event update loop, allowing the render thread to
        // never stall on waiting

        // TODO: NOTE: THIS MUST BE A SINGLE OBJECT THAT IS EXCHANGED
        // for it to be effectivly synchonized all outgoing events/effects _MUST_ happen
        // at the same time
        // for this to be lock free an entire object containing ALL the events that must
        // be synced must be exchanged

        // TODO: also note! this can be done for the processing of rendered out block
        // models!!
        // (it might be able to also be put in this thread, maybe? but is proabably
        // worth putting in own thread for latency reasons)
         ...
         // Geometry id metadata updates
            if (!this.geometryManager.getUpdateIds().isEmpty()) {
                ...
                while (iter.hasNext()) {
                    ...
                    // Geometry buffer is index of 1, so mutate to put it in that location, it is
                    // also 32 bytes, so needs to be split into 2 separate scatter writes
    ...
    public void tick(GlBuffer nodeBuffer, NodeCleaner cleaner) {// TODO: dont pass nodeBuffer here??, do something else
                                                                // thats better
        ...
        {// Update basic geometry data
            ...
            if (!upload.dataUploadPoints.isEmpty()) {
                ...
                UnsafeUtil.memcpy(upload.scratchHeaderBuffer.address, UploadStream.INSTANCE.getBaseAddress() + ptr,
                        copies * 16L);
                UnsafeUtil.memcpy(upload.scratchDataBuffer.address,
                        UploadStream.INSTANCE.getBaseAddress() + ptr + copies * 16L, scratchSize);
                UploadStream.INSTANCE.commit();// Commit the buffer

                this.multiMemcpy.bind();
                glBindBufferRange(GL_SHADER_STORAGE_BUFFER, 0, UploadStream.INSTANCE.getRawBufferId(), ptr,
                        copies * 16L);
                glBindBufferRange(GL_SHADER_STORAGE_BUFFER, 1, UploadStream.INSTANCE.getRawBufferId(),
                        ptr + copies * 16L, scratchSize);
                glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2,
                        ((BasicSectionGeometryData) this.geometryData).getGeometryBuffer().id);
        ...
        if (!results.scatterWriteLocationMap.isEmpty()) {// Scatter write
            ...
            MemoryUtil.memCopy(results.scatterWriteBuffer.address, UploadStream.INSTANCE.getBaseAddress() + ptr,
                    streamSize);
            UploadStream.INSTANCE.commit();// Commit the buffer
            ...
            glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2,
                    ((BasicSectionGeometryData) this.geometryData).getMetadataBuffer().id);
    ...
    public void addDebug(List<String> debug) {
        debug.add("UC/GC: " + (this.getUsedGeometryCapacity() / (1 << 20)) + "/"
    ...
    public void worldEvent(WorldSection section, int flags, int neighborMask) {
        // If there is any change, we need to clear the geometry cache before emitting
        // update
    ...
    // Results object, which is to be synced between the render thread and worker
    // thread
    private static final class SyncResults {
        ...
        // ensureExtra is used to ensure that allocations are "effectivly" in the same
        // memory block (kinda?)
        public long getScatterWritePtr(int location, int ensureExtra) {
            ...
            if (loc == -1) {// Location doesnt exist, create it
                ...
                MemoryUtil.memPutInt(this.scatterWriteBuffer.address + (chunkBase * 16L) + (innerId * 4L), location);// Set
                                                                                                                     // the
                                                                                                                     // write
                                                                                                                     // location
    ...
    private static class ComputeMemoryCopy {
        ...
        public void remove(int point) {
            ...
            // Else: we need to move the ending upload header from the end to where the
            // freed point was
            int endingPoint = MemoryUtil
                    .memGetInt(this.scratchHeaderBuffer.address + this.dataUploadPoints.size() * 16L + 4);
            ...
            MemoryUtil.memPutLong(A, 0);
            MemoryUtil.memPutLong(B + 8, MemoryUtil.memGetLong(A + 8));
            MemoryUtil.memPutLong(A + 8, 0);
        ...
        public void upload(int point, MemoryBuffer data) {
            ...
            if (header != -1) {
                ...
                if (pSize == elemSize) {
                    // The data we are replacing is the same size, so just overwrite it, this is the
                    // easiest
                ...
                long headerPtr = this.scratchHeaderBuffer.address + header * 16L;// Header resize has happened so this
                                                                                 // is a stable address
