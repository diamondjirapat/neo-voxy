...
public class RenderGenerationService {
    ...
    /*
     * public RenderGenerationService(WorldEngine world, ModelBakerySubsystem
     * modelBakery, ServiceManager sm, boolean emitMeshlets) {
    ...
    public RenderGenerationService(WorldEngine world, ModelBakerySubsystem modelBakery, ServiceManager sm,
        ...
        }, 10, "Section mesh generation service", () -> modelBakery.getProcessingCount() < 400
                || RenderGenerationService.MESH_FAILED_COUNTER.get() < 500);
    }
    ...
    private void computeAndRequestRequiredModels(IntOpenHashSet seenMissedIds, WorldSection section) {
        // Know this is... very much not safe, however it reduces allocation rates and
        // other garbage, am sure its "fine"
        final var factory = this.modelBakery.factory;
    ...
    private void processJob(RenderDataFactory factory, IntOpenHashSet seenMissedIds) {
        ...
        {// Remove the task from the map, this is done before we check for null sections
         // as well the task map needs to be correct
            ...
            if (task != null) {
                ...
                if (e.isIdBlockId) {
                    ...
                    if (!this.modelBakery.factory.hasModelForBlockId(e.id)) {
                        if (seenMissedIds.add(e.id)) {
                            if (this.modelBakery.requestBlockBake(e.id)) {
                                Logger.info("Mesh generation delayed: requesting model for block ID " +
                                        e.id + " (retry path)");
                            }
                        }
                    }
                ...
                if (task.hasDoneModelRequestInner && task.hasDoneModelRequestOuter) {
                    ...
                    if (!task.hasDoneModelRequestInner) {
                        // The reason for the extra id parameter is that we explicitly add/check against
                        // the exception id due to e.g. requesting accross a chunk boarder wont be
                        // captured in the request
                        if (e.auxData == null)// the null check this is because for it to be, the inner must already be
                                              // computed
                    ...
                    task.addin = WorldEngine.getLevel(task.position) > 2 ? 1 : 0;// Single time addin which gives the
                                                                                 // models time to bake before the task
                                                                                 // executes
                }

                // Keep the lock on the section, and attach it to the task, this prevents
                // needing to re-aquire it later
                if (task.section == null) {
    ...
    public void addDebugData(List<String> debug) {
        ...
        debug.add("RSSQ/TFC: " + this.taskQueueCount.get() + "/" + MESH_FAILED_COUNTER.get());// render section service
                                                                                              // queue, Task Fail
                                                                                              // Counter
